...
void audio_task::run() {
	...
	// configure the parameters for this audio worker
	{
		...
		input_queue = new user_queue_t<token_t>(hw_buf_pool, io_payload_size);

		// Allocate output queue
		output_queue = new user_queue_t<token_t>(hw_buf_pool, io_payload_size);

		// filter payload size for audio
		flt_payload_size = io_payload_size + 2;

		// Allocate output queue
		filter_queue = new user_queue_t<token_t>(hw_buf_pool, flt_payload_size);
	}
	...
	handle_sync();
	DEBUG(
		printf("\n[AUDIO thread %d] printing accel_handle %p:\n", thread_id, accel_handle);
		accel_handle->print();
	);
