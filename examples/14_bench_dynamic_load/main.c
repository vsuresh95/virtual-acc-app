...
int main(int argc, char **argv) {
    ...
    unsigned *periods;
    unsigned sleep_seconds = n_threads == 1 ? 1 : 2;
    switch (test_type) {
        case 0: {
            // Light workload
            model_list = light_models;
            periods = light_period;
            break;
        }
        case 1: {
            // Heavy workload
        ...
        case 2: {
            ...
            periods = mixed_period;
            sleep_seconds = n_threads == 1 ? 2 : 3;
            num_epochs = 60;
            break;
        }
        default: break;
    }
    printf("[FILTER] Starting app %s %s, %s for test %d for %d threads (offset %d)\n", sm_print, mozart_print, vam_print, test_type, n_threads, th_offset);
    ...
    for (unsigned epoch = 0; epoch < num_epochs; epoch++) {
        ...
        for (unsigned i = 0; i < n_threads; i++) {
            if (epoch > 1 && (i+th_offset) == epoch % MAX_THREADS) {
                args->cmd_period = (unsigned) (scale_factor[epoch][i+th_offset] * (float) periods[i+th_offset]) * n_threads;
                LOW_DEBUG(printf("[MAIN] th %d period %d\n", args->t_id, args->cmd_period);)
            } else {
                args->cmd_period = 0;
            }
        ...
        for (unsigned i = 0; i < n_threads; i++) {
            ...
            printf("%0.2f, %0.2f%%, ", throughput, util * 100);
            #else
