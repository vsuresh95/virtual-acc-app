...
void vam_worker::configure_accel(virtual_inst_t *accel_handle) {
    ...
    //     // Or, the capability is composable but we allocated a monolithic accelerator to it.
        ...
        if (virt_to_phy_mapping[accel_handle][0] == NULL) {
            DEBUG(printf("PHYSICAL ACCEL for thread %d is NULL\n", accel_handle->thread_id);)
        } else {
            DEBUG(printf("PHYSICAL ACCEL for thread %d is %p\n", accel_handle->thread_id, virt_to_phy_mapping[accel_handle][0]);)
        }
    ...
    //         }
    //         case AUDIO_FFI: {
                audio_inst_t* audio_handle = (audio_inst_t *) accel_handle;
